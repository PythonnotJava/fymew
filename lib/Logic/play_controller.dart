...
class PlayerController extends ChangeNotifier {
  ...
  /// 启动时的音频模式
  final int onlyStartMode = mgrUserData['mode'];

  bool get isPaused => !audioPlayer.state.playing;
  PlayList get playLists => _playlist;
  bool get isPlaying => audioPlayer.state.playing;
  ...
  Future<void> loadInitialPlaylistAndRecord() async {
    ...
    if (initInfoState == 4) {
      ...
      await audioPlayer.open(Media(_currentMusicInfo.musicPath), play: false);
  ...
  Future<void> _configureAudioSession() async {
    ...
    /// 使用 AudioSessionConfiguration.music() 适用于播放音乐
    await session.configure(const AudioSessionConfiguration.music());

    /// 监听中断事件 (可选，但推荐手动控制)
    /// 监听其他应用（如电话、导航）引起的焦点变化
    final int mode = onlyStartMode;
    debugPrint("启动时音频模式：$mode");
    if (mode == 0) {
      session.interruptionEventStream.listen((event) async {
        debugPrint("监听中断事件……");
        if (event.begin) {
          debugPrint("中断开始: ${event.type}");
          ...
          /// 中断结束
          debugPrint("中断结束: ${event.type}");
          if (event.type == AudioInterruptionType.pause) {
            debugPrint("阻断模式下需要手动启动");
          } else {
            debugPrint("未知事件");
          }
        }
  ...
  /// 信息到注册系统音频中
  void toMediaItem(MusicInfoReader info) {
    if (onlyStartMode != 3) {
      audioHandler.addMediaItem(
        MediaItem(
          id: info.musicPath,
          title: info.title,
          artist: info.artist,
  ...
  Future<void> justUpdateMusicAndIndex(MusicInfoReader info, int index) async {
    ...
    await audioPlayer.open(Media(info.musicPath), play: false);
    notifyListeners();
  }
