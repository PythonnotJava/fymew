...
class PlayerController extends ChangeNotifier {
  ...
  bool get isPaused => !audioPlayer.playerState.playing;
  ...
  bool get isDefaultMusic => currentMusicInfo.musicPath == defaultMusicPath;
  ...
  (bool, MusicInfoReader) get randomInfo => playTotalLib.isEmpty
      ? (true, MusicInfoReader.defaultReplace)
  ...
  /// 播放器停止
  Future<void> stopAudioPlayer() async => audioPlayer.stop();
  ...
  /// 加入大量歌曲到乐库
  Future<void> addMoreMusicsToPlaylistLib(
    List<MusicInfoReader> infos, {
    bool notify = false,
  }) async {
    playTotalLib.addAll(infos);
    if (notify) {
      notifyListeners();
    }
  }
  ...
  /// 每次点播的时候，都会进行记录写入配置
  Future<void> writeInfoToRecord(MusicInfoReader info) async {
    mgrPreMusicData['currentMusic'] = info.musicPath;
    mgrPreMusicData['infopkl'] = info.encodeName;
  ...
  Future<void> loadInitialPlaylistAndRecord() async {
    ...
    if (initInfoState == 4) {
      ...
      try {
        ...
        _currentIndex = -1;
        _currentMusicInfo = MusicInfoReader.defaultReplace;
        debugPrint('意外捕捉，原因：$e');
      }

      await audioPlayer.setFilePath(_currentMusicInfo.musicPath);
  ...
  /// Constructor
  PlayerController(AudioSession audioSession) {
    /// 监听播放状态变化
    audioPlayer.playerStateStream.listen((PlayerState state) {
      /// 仅根据播放器的真实状态更新
      debugPrint("播放器状态变化: ${state.processingState}, playing=${state.playing}");
      ...
      _stateStreamController.add(playbackState);
  ...
  /// ---------------------------------播放相关逻辑----------------------------------------
  /// 创建 MediaItem 并广播
  void createMediaItemToAppend(MusicInfoReader info) {
    final mediaItem = MediaItem(
      id: info.musicPath,
      title: info.title,
      artist: info.artist,
      duration: Duration(seconds: info.musicLength),
      artUri: Uri.file(info.coverPath),
    );
    _mediaItemStreamController.add(mediaItem);
  }

  /// 仅仅是更新索引和信息
  Future<void> justUpdateMusicAndIndex(MusicInfoReader info, int index) async {
    _currentIndex = index;
    _currentMusicInfo = info;
    await audioPlayer.setFilePath(info.musicPath);
    createMediaItemToAppend(info);
    notifyListeners();
  }
  ...
  Future<void> playQueueMusic(int index, MusicInfoReader info) async {
    _currentMusicInfo = info;
    _currentIndex = index;
  ...
  /// 辅助：转换状态（用于 audio_service 的 PlaybackState）
  static AudioProcessingState _getProcessingState(ProcessingState state) {
    switch (state) {
  ...
  Future<void> playPause() async {
    ...
    currentState.playing ? audioPlayer.pause() : audioPlayer.play();
    debugPrint("手动设置 -- ${!currentState.playing ? '继续播放' : '暂停'}");
    notifyListeners();
  }
}
